...
export default function Long({
  ...
  const { upReturn, downReturn, upValue, downValue } = useMemo(() => {
    const upReturn = new Percent(3);
    const downReturn = new Percent(-3, 4);

    if (!input || input.equalTo(0)) return { upReturn, downReturn };

    const upValue = input.multiply(upReturn);
    const downValue = input.multiply(new Percent(1).add(downReturn));
